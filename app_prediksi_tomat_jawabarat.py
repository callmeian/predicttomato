...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # =========================
            # VALIDASI TAHUN MASA DEPAN
            # =========================
            tahun_terakhir_dataset = int(df["tahun"].max())

            if (df_input["tahun"] <= tahun_terakhir_dataset).any():
                st.error(
                    f"File CSV harus berisi tahun prediksi masa depan (> {tahun_terakhir_dataset})"
                )
                st.stop()
            ...
            cek_logis("luas_areal_tanam")
            cek_logis("luas_panen")
            cek_logis("curah_hujan")
            cek_logis("kelembapan")
            ...
            # =========================
            # FILTER TAHUN MASA DEPAN
            # =========================
            tahun_terakhir_dataset = int(df["tahun"].max())

            df_future = df_input[df_input["tahun"] > tahun_terakhir_dataset].copy()

            if df_future.empty:
                st.warning(
                    f"⚠️ Tidak ada data prediksi tahun masa depan.\n"
                    f"Tahun harus lebih besar dari {tahun_terakhir_dataset}."
                )
                st.stop()
            ...
            df_future = df_future[
                [
                    "nama_kabupaten_kota",
                    "tahun",
                    "prediksi_produksi_tomat"
                ]
            ]

            st.subheader(
                f"📊 Prediksi Tomat Masa Mendatang "
            )
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("Prediksi Tomat Masa Mendatang")
    ...
    if predict_button:
        ...
        try:
            ...
            st.subheader("📊 Hasil Prediksi Produksi Tomat")
            st.success(
                f"Perkiraan produksi tomat tahun **{tahun_prediksi}**: "
                f"**{pred_value:,.2f} ton**"
            )
